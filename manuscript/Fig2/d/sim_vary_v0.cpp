...
{
    int V0 = atoi(argv[3]);                 // treatment efficacy
    ...
    for (r_ind=0; r_ind<repeats; r_ind++)
    {
        ...
        success += RUN(V0,scenario,model);         // updating survival counter (+1 if resistant strain survived)
    }
    
    double avg;                                  // empirical survival probability of R
    avg = (double)success/(double)repeats;
    
    ofstream file ("surv_HN_eps_0_sc_" + std::to_string(scenario) + "_model_" + std::to_string(model) + ".txt", ios::app);   // file output, average
    file << V0;
    file << ",";
    file << avg;  
    file << "\n";
...
int RUN(int V0, int scenario, int model)
{
