...
int RUN(double eps, int scenario, int model)
{
            ...
            while (update == 0)
            {
                if (rand1 < accumulate(rates,rates+ind+1,0.)/accumulate(rates,rates+5,0.))
                {
                    ...
                    else if (ind == 2)
                    {
                        ...
                        if (scenario == 0)
                        {
                            V = V + gsl_ran_poisson(r,mu*(1-eps)*B);
                            W = W + gsl_ran_poisson(r,(1-mu)*(1-eps)*B)
                        }
    ...
    {
        return_value = 1;
        ofstream file ("ext_LN_V0_1_eps_" + to_string(eps) + "_sc_" + to_string(scenario) + "_model_" + to_string(model) + ".txt", ios::app);   // file output, average
        file << t;  
        file << "\n";
        file.close();
    }
