...
for j in range(len(eps_data)):
    ...
    for k in range(len(V0)):
        ...
        print(np.percentile(data,95))
        print("\n")
        data = genfromtxt('esttime_LN_V0_%s_eps_%s_sc_2_model_1.txt' % (V0[k], eps_data[j]), delimiter=',')
        print(np.percentile(data,5))
        print(np.percentile(data,50))
        print(np.percentile(data,95))
        print("\n")
...
for j in range(len(eps_data)):
    ...
    for k in range(len(V0)):
        ...
        print(np.percentile(data,95))
        print("\n")
        data = genfromtxt('esttime_HN_V0_%s_eps_%s_sc_2_model_1.txt' % (V0[k], eps_data[j]), delimiter=',')
        print(np.percentile(data,5))
        print(np.percentile(data,50))
        print(np.percentile(data,95))
        print("\n")