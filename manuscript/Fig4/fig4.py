...
def euler(sc,V0,det):
    ...
    for i in range(len(eps)):
        ...
        if (sc == 3):
            ...
            c2 = c/(1-eps[i])
        #   
        phi = 1-(c2/(c2+beta2*T)+delta2/p2)**V0
        #
        t = 0
        #
        if (det == 0):
        ...
        if (phi>0 and i < 76):
            while (V >= thresh):
            	...
            	Vnew = V + p2 * I * dt - c2 * V * dt - beta2 * V * T * dt
            	t += dt
            	#
            	T = Tnew
            	E = Enew
...
for j in range(len(eps_plot)):
    for l in range(len(V0)):
        data = genfromtxt('esttime_HN_V0_%s_eps_%s_sc_0_model_1.txt' % (V0[l], eps_label[j]), delimiter=',')
        #plt.plot(eps_data[j],np.mean(data),'o',color='C0',markersize=10)
        error = np.transpose([[np.percentile(data,10),np.percentile(data,90)]])
        plt.errorbar(eps_plot[j],np.mean(data),yerr=error,fmt='o',color='C0', markersize=10, elinewidth=2,capsize=4)
        data = genfromtxt('esttime_HN_V0_%s_eps_%s_sc_1_model_1.txt' % (V0[l], eps_label[j]), delimiter=',')
        #plt.plot(eps_data[j],np.mean(data),'o',color='C1',markersize=10)
        error = np.transpose([[np.percentile(data,10),np.percentile(data,90)]])
        plt.errorbar(eps_plot[j]+0.01,np.mean(data),yerr=error,fmt='o',color='C1', markersize=10, elinewidth=2,capsize=4)
        data = genfromtxt('esttime_HN_V0_%s_eps_%s_sc_3_model_1.txt' % (V0[l], eps_label[j]), delimiter=',')
        #plt.plot(eps_data[j],np.mean(data),'o',color='C2',markersize=10)
        error = np.transpose([[np.percentile(data,10),np.percentile(data,90)]])
        plt.errorbar(eps_plot[j]+0.02,np.mean(data),yerr=error,fmt='o',color='C2', markersize=10, elinewidth=2,capsize=4)
