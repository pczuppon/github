...
// g++ -std=c++11 sim_virus_estab.cpp `pkg-config --libs gsl`  command for compiling
...
// Main code
int RUN(double,int,int,int);

int main(int argc,char *argv[])
{
    double eps = 0.75;                           // treatment efficacy
    ...
    while (success < 50)
    {
        ...
        success+=RUN(eps,scenario,model,success);         // updating survival counter (+1 if resistant strain survived)
        r_ind++;
    }
...
int RUN(double eps, int scenario, int model,int s_ind)
{
    ...
    {   
        ofstream file ("traj_time_LN_eps_075_ind_" + to_string(s_ind) + "_sc_" + to_string(scenario) + ".txt");   // file output, average
