...
// g++ -std=c++11 sim_combination.cpp `pkg-config --libs gsl` command for compiling
...
{
    double eps = atof(argv[2]);                 // treatment efficacy
    int scenario = atoi(argv[1]);            // scenario: 0 = burst-burst, 1 = burst-inf, 2 = inf-inf
    ...
    for (r_ind=0; r_ind<repeats; r_ind++)
    {
        ...
        success += RUN(eps,scenario);         // updating survival counter (+1 if resistant strain survived)
    }
