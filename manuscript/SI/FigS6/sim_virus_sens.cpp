...
// g++ -std=c++11 sim_virus_sens.cpp `pkg-config --libs gsl` command for compiling

using namespace std;

// Within-host parameters
#define R0 12.2             // R0 value
#define k 5.0           // eclipse phase: I_1 -> I_2
#define delta 0.63        // cell death
#define c 10.0           // virus clearance
#define p 21.8             // continuous viral production
...
double beta = R0*c*delta/((double)T0*(p-R0*delta));    // virus infectivity
...
int main(int argc,char *argv[])
{
    ...
    ofstream file ("surv_HR_LN_V0_10_sc_" + std::to_string(scenario) + "_model_" + std::to_string(model) + ".txt", ios::app);   // file output, average
    file << eps;
    file << ",";
    file << avg;  
    file << "\n";
